...
> [!INFO] Disclaimer
>
> As with [my previous post](/blog/managemyhealth-data-breach-recap), I have no special insight or behind-the-scenes knowledge of the ManageMyHealth breach, nor am I affiliated with any entities who may have assisted in the breach investigation. While I've strived to be as accurate as I can be, I'm neither a reporter nor a cybersecurity professional by day. I've tried to explicitly indicate where I'm diving into my own opinion and/or speculating but as always, carry out your own due diligence when trusting strangers on the internet.

It feels weird to think that [my previous post](/blog/managemyhealth-data-breach-recap) was just under a month ago because so much wacky stuff happened as a result, with the most important being that I certainly learned a lot of new things!

For those only familiar with that earlier post, I had been keeping tabs on this whole saga [over on Bluesky](https://bsky.app/profile/utf9k.net) (and still am!).

There are still a few loose threads that I haven't seen anyone dig into which I think are important to capture in a more permanent medium than Bluesky posts but first we'll do a quick recap of the events since the first post.

It won't cover every detail and if you want more, there's now a [Wikipedia](https://en.wikipedia.org/wiki/ManageMyHealth_data_breach) covering the saga at a high level.

## Recapping from January 4th

### Deadlines came and went

Around the time of the previous post, ManageMyHealth had acknowledged that a breach had occurred, two[^erroneous] sets of breach samples had been published, and the original deadline of January 16th, 2026 was still looming.
...
Going off of the timestamp of the deadline message, the exact time would have been 5:37am on January 6th, only for it to come and go with no explanation from either Kazu or ManageMyHealth.

Later on in the day, Kazu [confirmed to The Post](https://www.thepost.co.nz/nz-news/360926388/will-manage-my-health-hacker-release-confidential-patient-data-today) that there was a new round of negotiations, which I clarified meant there was a new deadline of 5am on January 9th.
...
It was around the afternoon of January 6th that my own interactions with Kazu started trailing off, as they had stopped replying to my messages.
...
By Wednesday January 21st, the Privacy Commissioner had also [announced](https://www.privacy.org.nz/tuhono-connect/statements-media-releases/privacy-commissioner-inquiry-into-manage-my-health-breach/) that he would be carrying out his own investigation in regards to the privacy aspects of how ManageMyHealth had been operating, such as what sort of contractual arrangements were in place for data sharing, and whether they were in compliance with the Privacy Act.
...
Both investigations are expected to produce something resembling a final report by April 30th, 2026, with the technical assurance part of the Ministry's review due on February 28th, 2026.

If you're interested in this stuff, I recommend reading the Terms of Reference as they're only a few pages and fairly accessible, as far as not needing some sort of legal degree to decipher. They are still technical in nature and might be a bit confusing to non-technical folk.
...
The [judge's decision](https://www.justice.govt.nz/jdo_documents/workspace___SpacesStore_78cb64bc_0e8f_4332_9851_5d51b9992657.pdf)[^publication] contains the text of the injunction on Pages 9 and 10 but they then need to get a "sealed order" which is a fancy way of saying [a copy that is stamped with the official seal of the High Court](https://cdn.utf9k.net/blog/mmh-breach-two/010%20Sealed%20Order%206%20January%202026.pdf).

Personally, the term "sealed order" made me think they were referring to some sort of secret court order that was not available to the public. That would be silly because then how are you meant to know what you're meant to be doing (or not).

As far as using the sealed order, a lawyer[^solicitor] will then present the order to whomever it applies to, usually in the form of a PDF sent via email. If you read the injunction, it applies to "Unknown Defendants" so it can be presented to anyone suspected of holding data from the breach. Understandably, who that might be is mostly unknowable at the time of filing.
...
There are a bunch of things that an affected "agency" (ManageMyHealth) have to do such as notifying [the Office of the Privacy Commissioner](https://www.privacy.org.nz/) but most importantly, they have to notify every single person affected.

This isn't always possible at first so Section 115 allows for giving a public notice as a temporary measure while they sort out who exactly needs to be notified and for the first week or two, we were in the "public notice" phase.

At the time of writing, we still have not reached 100% of individuals notified and progress seems to have crawled to a snail's pace with increasingly vague wording. Here's a quick list of the various wording that I've seen:
...
As mentioned in Section 115 of the Privacy Act 2020, individuals must be notified "as soon as practicable" but there is no hard deadline or even a guideline of what "practicable" can be expected to mean.
...
There have been a couple of events that have been presented as hard factors but upon digging a bit further, I think they ranged from "Take this with a grain of salt" to "This seems like a bunch of nonsense".
...
In it, they mention that unnamed sources have "confirmed" a data breach in the ManageMyHealth app and that their source (now singular?) "made contact with NZ Gov officials on June 22, 2025" only to receive "no official response to action".

They include a screenshot of an email sent to `support@managemyhealth.co.nz` and `enquiries@privacy.co.nz` that was also [BCC-ed](https://en.wikipedia.org/wiki/Blind_carbon_copy) to `cmluxon@[redacted].com`, the latter of which probably came from [this website](https://contactout.com/christopher-luxon-72919).
...
This timeline would seem to line up with what both ManageMyHealth and the Office of the Privacy Commissioner have mentioned.

The article then presents screenshots of a PDF that their source had apparently sent as part of the email mentioned above.
...
For those following along with the whole LLM bubble/boom, you may raise an eyebrow or two given that LLMs love to generate lists that follow a key-value style, where the key is bold and the value is regular text.

Most of this wording... seems generated by an LLM. I would not expect a real penetration test report to say something like "Presence of both 'http' and 'https' URLs suggest legacy or mixed-protocol vulnerabilities".

A real report would either say there is a finding or there is not. Padding it with "this could be a problem" defeats the whole point but LLMs love to use that sort of couched language.[^couched]
...
What we're presented with is lists of URLs that follow the general format of `https://managemyhealth.co.nz:/:<username>:<password>`.
...
For the uninitiated, search engines work by crawling websites and indexing URLs that exist on pages. The more links your website receives, the more "authoritative" it is, and the higher it ranks. There is no rule, however, that a URL necessarily [has to be valid to be indexed](https://webmasters.stackexchange.com/a/63461) by a search engine.

To put it in other words, crafting a URL that contains user information is the technical equivalent of projecting my credit card number on the side of the ManageMyHealth office building and then complaining that they're leaking my credit card number.
...
There is a [small cottage industry](https://www.troyhunt.com/beg-bounties/) of people who "discover" these sorts of security issues, probably nowadays by just copy-pasting whatever an LLM tells them, and then mailing the affected "victim" with their findings.

It's a valid question to ask where these alleged usernames and passwords come from and presumably they came from a previous breach.
...
What can sometimes happen is that some websites don't follow this standard and return a [200 OK](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes#200) for anything and everything. It doesn't take much to imagine a hacker taking a previously leaked set of usernames and passwords, loading them into some sort of automatic tool, pointing it at the ManageMyHealth marketing website, receiving back "Wow, these are all valid URLs" and then believing that this means they've successfully hacked into a bunch of accounts.

As for the email origin, I squinted at the various George News screenshots and was able to figure out about 10 different emails which I ran through [HaveIBeenPwned](https://haveibeenpwned.com/) and almost all of them returned hits.
...
These breaches, and many others, were then continuously recompiled into [many](https://haveibeenpwned.com/Breach/DataTrollStealerLogs) [different](https://haveibeenpwned.com/Breach/SynthientStealerLogThreatData) [sets](https://haveibeenpwned.com/Breach/AlienStealerLogs) [that](https://haveibeenpwned.com/Breach/StealerLogsJan2025) [continuously](https://haveibeenpwned.com/Breach/TelegramCombolists) [get](https://haveibeenpwned.com/Breach/NazApi) re-breached and recycled until someone used one of these mega-sets against the ManageMyHealth marketing website and claimed an early victory for their minimal efforts.
...
This struck me as a bit bizarre as I would have thought the easiest way to not spook someone you're investigating is by simply not blasting it out to the public with a megaphone.

Mike Jagusch, the Chief Operating Officer at the National Cyber Security Centre acknowledged that they were aware of the claims and said:
...
From what I've pieced together, this might look like using OSINT techniques such as chaining together user profiles (i.e. Roblox -> Spotify -> Instagram) and other tricks to identify alleged offenders.

Their current Lead Investigator detailed an incident on LinkedIn, prior to their joining IOC3 as a volunteer, where they had infiltrated a right wing Discord server and compiled enough evidence to send to their employer.

I don't know, and am not suggesting that these are the same techniques used by IOC3, as they may only be concerned with identifying individuals and then filing police reports. Their involvement may also just be guiding individuals through filing their own reports via various game platforms.

Anyway, putting aside all that, I'm not sure that I would recommend Ticket Tool as somewhere to store case files that may relate to horrors suffered by minors. You probably want to at least have some sort of vendor contract to guarantee that data shared on your ticketing platform is secure, rather than just using a publicly available Discord bot.
...
There appears to be at least one person involved who has been doing policy type roles for quite a few years but as with prestigious sounding names like "International Centre", it's possible that a few of those previous roles equate to writing articles from the corner of a local Starbucks. Nothing wrong with it to be clear!
...
From all this, while I'm not a particular fan of invoking prestigious sounding names and titles, I think the generous interpretation is that we're seeing an early stage startup, with all the mistakes that come with being relatively young.

Given the nature of the space they're working in, I would expect to see a lot more rigour behind establishing a solid web presence with some official information to look at because my initial impression seeing a series of discarded startups raised a lot of red flags, and still does to some extent.
...
I feel pretty dumb for doing some first principles guesses in my last post when I should have just made an account and poked around. I had imagined that I would actually need to start funnelling all of my health records over to them but it turns out that it's pretty trivial to make an account.

Making an account and navigating to the Health Modules section, it doesn't seem to take more than 5 minutes to find a viable avenue.

![](https://cdn.utf9k.net/blog/mmh-breach-two/mmh-docs-upload.png "I am enjoying uploading an image of a wiener dog into the ManageMyHealth portal")
...
Note how the document ID is an incrementing number. We can probably assume that the last uploaded document was given the ID `477586` and the next uploaded document will be given the ID `477588`. That doesn't necessarily mean we can access or modify those documents however.

Carrying on, the URL above retrieves the following metadata in order to render the details page:
...
```json
{
  ...
  "IsActive": false,
  "UserProfileId": 0,
  "PatientID": "330948f9-5614-4a6e-9fad-253896e26a42",
  "CreatedBy": "330948f9-5614-4a6e-9fad-253896e26a42",
  "Source": "PATIENT"
}
```
...
The employee credential theory avenue would also seem to contradict only Health Documents being affected. I would expect employee credentials to have much wider access to the ManageMyHealth systems, and we'd be talking about an even broader scope as far as what was stolen.
...
It may have been the case that Kazu just signed up for an account or they may have tried stolen username/password combinations from a previous breach until they found one that worked.

Either way, I don't have my fingers crossed that we'll learn something super exciting.

### My guess as to whether the ransom was paid

Personally, I think the ransom was paid but that's just my personal opinion and not a provable fact.
...
There has been no mention of the breach from either side but I think we can assume that part of paying the ransom includes silence on the part of Kazu.
...
Finally, there's this message. At first, I thought they meant they were signing off for the day.

![](https://cdn.utf9k.net/blog/mmh-breach-two/kazu-signoff.jpeg "Kazu thanks the author for their hard work")

For context, the "hard work" they're referring to is trying to share what I learned about the whole saga, which became a whole second job around when the first deadline was brought forward.

I don't doubt it benefited them, having more attention at the same time. At least up until it became too much I suppose.
...
I wrote the first ManageMyHealth blog post between 12am - 4am on the morning of Sunday 4th January because I would get charged $10 if a new post didn't appear in my RSS feed for Beeminder to automatically verify.[^cheating]
...
[^changedetection]: I don't have links for these updates, and they weren't captured by the Wayback Machine but they were captured by the instance of [changedetection.io](https://github.com/dgtlmoon/changedetection.io) that I run at home.
...
[^x]: Yes, the infamous Elon Musk pedophilia generation website. I still need to delete my account. Coding agents are having a moment in early 2026 which I still need to write about and a lot of LLM research types [are on there](https://x.com/bcherny/status/2007179832300581177) which is mostly why I still haven't gotten around to deleting my account. Fuck Generative AI for art to be clear though. I will continue to buy and support real art as I have for many years. I think the whole thing is a bubble that does not require terraforming small rural towns into data centres but there is also value in "[automatic programming](https://antirez.com/news/159)". Surely we can scale down this whole operation to what's actually useful and get rid of the [Microsoft Copilot](https://en.wikipedia.org/wiki/Microsoft_Copilot) bullshit enterprise shit that no one cares to buy. That seems to be powering most of the worst aspects of this whole thing. A story for another day.
