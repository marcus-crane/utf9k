(() => {
  ...
  window.addEventListener("DOMContentLoaded", function() {
    ...
    if (lookup === null) {
      fetch("/search.json").then((res) => res.json()).then((pages) => {
        lookup = pages;
    ...
    function getTags(lookup2) {
      let tags = [];
      for (let page of lookup2) {
        tags = [...page.tags, ...tags];
      }
      return tags;
    }
    function handleInput() {
      ...
      for (let page of lookup) {
        ...
        if (page.content.toLowerCase().includes(searchTerm) || checkTagContent(page.tags, searchTerm) || page.title.toLowerCase().includes(searchTerm) || page.slug.toLowerCase().includes(searchTerm)) {
          question.style.display = "list-item";
        } else {
